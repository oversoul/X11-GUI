...
static uint32_t xcb_color_to_uint32(xcb_render_color_t rgb) {
  uint32_t sm1 = 65536 - 1; // from 2^16
  uint32_t scale = 256;     // to 2^8

  return (uint32_t)(((double)rgb.red / sm1 * (scale - 1)) * scale * scale) +
         (uint32_t)(((double)rgb.green / sm1 * (scale - 1)) * scale) +
         (uint32_t)(((double)rgb.blue / sm1 * (scale - 1)));
}

xcb_pixmap_t xcbft_create_text_pixmap(xcb_connection_t *c, utf_holder text, xcb_render_color_t text_color,
                                      xcb_render_color_t background_color, xcbft_patterns_holder font_patterns,
                                      long dpi) {
  xcb_pixmap_t pmap, resize_pmap;
  xcb_screen_t *screen;
  screen = xcb_setup_roots_iterator(xcb_get_setup(c)).data;
  xcbft_face_holder faces;
  double pix_size = 12;
  uint32_t mask = 0;
  uint32_t values[2];
  xcb_gcontext_t gc;
  ...
  xcb_rectangle_t rectangles[] = {{.x = 0, .y = 0, .width = width, .height = height}};
  ...
  FT_Vector advance = xcbft_draw_text(c, pmap, 0.2 * pix_size, 0.2 * pix_size + pix_size, // x, y
 ...
 */
FcPattern *xcbft_query_fontsearch(FcChar8 *fontquery) {
  FcBool status;
 ...
 */
xcbft_face_holder xcbft_query_by_char_support(FcChar32 character, const FcPattern *copy_pattern, long dpi) {
  FcBool status;
  FcResult result;
  FcCharSet *charset;
  FcPattern *charset_pattern, *pat_output;
  xcbft_patterns_holder patterns;
  xcbft_face_holder faces;
...
xcbft_patterns_holder xcbft_query_fontsearch_all(FcStrSet *queries) {
  xcbft_patterns_holder font_patterns;
  ...
  while ((fontquery = FcStrListNext(iterator)) != NULL) {
    ...
    if (font_pattern != NULL) {
      if (font_patterns.length + 1 > current_allocated) {
        current_allocated += 5;
        font_patterns.patterns = (FcPattern **)realloc(font_patterns.patterns, sizeof(FcPattern *) * current_allocated);
      }
...
xcbft_face_holder xcbft_load_faces(xcbft_patterns_holder patterns, long dpi) {
  int i;
  xcbft_face_holder faces;
  FcResult result;
  ...
  for (i = 0; i < patterns.length; i++) {
    ...
    // load the face
    error = FT_New_Face(library, (const char *)fc_file.u.s, fc_index.u.i, &(faces.faces[faces.length]));
    ...
    // pixel_size/ (dpi/72.0)
    FT_Set_Char_Size(faces.faces[faces.length], 0, (fc_pixel_size.u.d / ((double)dpi / 72.0)) * 64, dpi, dpi);
    if (error != FT_Err_Ok) {
      perror(NULL);
...
FcStrSet *xcbft_extract_fontsearch_list(const char *string) {
  ...
  while (token != NULL) {
    ...
    if (result == FcFalse) {
      fprintf(stderr, "Couldn't add fontquery to fontsearch set");
    }
    token = strtok(NULL, ",");
  }
...
void xcbft_patterns_holder_destroy(xcbft_patterns_holder patterns) {
  int i = 0;
...
void xcbft_face_holder_destroy(xcbft_face_holder faces) {
  int i = 0;
...
FT_Vector xcbft_draw_text(xcb_connection_t *c,  // conn
                          xcb_drawable_t pmap,  // win or pixmap
                          int16_t x, int16_t y, // x, y
                          utf_holder text,      // text
                          xcb_render_color_t color, xcbft_face_holder faces, long dpi) {
  ...
  if (error) {
    fprintf(stderr, "ERROR: %s : %d\n", "could not create picture", error->error_code);
  }
  ...
  xcbft_glyphset_and_advance glyphset_advance = xcbft_load_glyphset(c, faces, text, dpi);

  // we now have a text stream - a bunch of glyphs basically
  xcb_render_util_composite_text_stream_t *ts =
      xcb_render_util_composite_text_stream(glyphset_advance.glyphset, text.length, 0);
...
xcbft_glyphset_and_advance xcbft_load_glyphset(xcb_connection_t *c, xcbft_face_holder faces, utf_holder text,
                                               long dpi) {
  unsigned int i, j;
  int glyph_index;
  xcb_render_glyphset_t gs;
  xcb_render_pictforminfo_t *fmt_a8;
  xcbft_face_holder faces_for_unsupported;
  const xcb_render_query_pict_formats_reply_t *fmt_rep = xcb_render_util_query_formats(c);
  FT_Vector total_advance, glyph_advance;
  xcbft_glyphset_and_advance glyphset_advance;
  ...
  for (i = 0; i < text.length; i++) {
    for (j = 0; j < faces.length; j++) {
      glyph_index = FT_Get_Char_Index(faces.faces[j], text.str[i]);
      if (glyph_index != 0)
        break;
    }
    ...
    if (glyph_index != 0) {
      ...
      if (faces_for_unsupported.length > 0) {
        glyph_index = FT_Get_Char_Index(faces_for_unsupported.faces[0], text.str[i]);
      }
      if (glyph_index == 0) {
        if (faces_for_unsupported.length > 0) {
      ...
      if (faces_for_unsupported.length == 0) {
        fprintf(stderr, "No faces found supporting character: %02x\n", text.str[i]);
        // draw a block using whatever font
        glyph_advance = xcbft_load_glyph(c, gs, faces.faces[0], text.str[i]);
        total_advance.x += glyph_advance.x;
        total_advance.y += glyph_advance.y;
      } else {
        FT_Set_Char_Size(faces_for_unsupported.faces[0], 0,
                         (faces.faces[0]->size->metrics.x_ppem / ((double)dpi / 72.0)) * 64, dpi, dpi);

        glyph_advance = xcbft_load_glyph(c, gs, faces_for_unsupported.faces[0], text.str[i]);
        total_advance.x += glyph_advance.x;
        total_advance.y += glyph_advance.y;
      }
    }
...
FT_Vector xcbft_load_glyph(xcb_connection_t *c, xcb_render_glyphset_t gs, FT_Face face, int charcode) {
  uint32_t gid;
  int glyph_index;
  ...
  xcb_render_add_glyphs_checked(c, gs, 1, &gid, &ginfo, stride * ginfo.height, tmpbitmap);
